...
# Closing all plots in the start of the program.
# Initial and only API call of the program, alongside with key.
# Setting the limit to the data of 100000, the death data contains
# 10,000 plus data points.
plt.close('all')
cdc_api = Socrata('data.cdc.gov', 'Jx6TfqIGNRuPtssfGBZMkZe6l')
death_data = cdc_api.get('bi63-dtpu', limit=100000)

# Using pandas to build dataframe object, this nullified the need for
# multiple API calls. Setting deaths to int so they can be sorted later in
# program, and also removing all values pertaining to the column 'All causes'.
...
# Initializing the dataframes with each associated year in the dataframe, 1999-2017.
...
# Decision tree. Opens with the question what year your interested, the later defined finction asks for user input
# with specifics on what year they are interested in. After that there are more prompts for what state
# and table is delivered alongside a bar graph.
main_tree = \
